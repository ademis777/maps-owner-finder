...
type Analysis = {
  business: { name?: string; address?: string; phone?: string; website?: string; category?: string; mapsUrl: string };
  ...
  return <main className="shell">
    ...
    <section className="panel">
      ...
      {data && <div className="results">
        <article className="card"><h2>Business</h2><div className="grid">{Object.entries({ Name: data.business.name, Address: data.business.address, Phone: data.business.phone, Website: data.business.website, Category: data.business.category }).map(([label, value]) => <div className="field" key={label}><span className="label">{label}</span><span className="value">{value || "Not found"}</span></div>)}</div></article>
        <article className="card"><h2>Owner candidates</h2>{data.ownerCandidates.length === 0 ? <p>No owner was confirmed from the sources checked.</p> : data.ownerCandidates.map((candidate, index) => <div className="source" key={`${candidate.name}-${index}`}>
          <strong>{candidate.name}{candidate.title ? ` — ${candidate.title}` : ""}</strong><span className="badge">Confidence {candidate.confidence}%</span>
