  ...
  return <main className="shell">
    ...
    <section className="panel">
      ...
      {data && <div className="results">
        ...
        <article className="card ownerResult"><h2>Owner / decision maker</h2>{data.ownerCandidates.length === 0 ? <p>No owner was confirmed from the sources checked.</p> : data.ownerCandidates.map((candidate, index) => { const possiblePhones = candidate.contacts?.filter((contact) => contact.kind === "phone" && contact.contactType === "possible_direct") || []; const possibleEmails = candidate.contacts?.filter((contact) => contact.kind === "email" && contact.contactType === "possible_direct") || []; const directContacts = candidate.contacts?.filter((contact) => contact.contactType === "verified_direct" || contact.contactType === "possible_direct") || []; return <div className="source" key={`${candidate.name}-${index}`}>
          ...
          <h3>Direct professional contact</h3>
          {directContacts.length ? directContacts.map((contact, contactIndex) => <div className="source" key={`${contact.kind}-${contact.value}-${contactIndex}`}><strong>{contact.value}</strong><span className="badge">{contact.contactType} · Confidence {contact.confidence}%</span><div className="note">{contact.reason}</div><a href={contact.sourceUrl} target="_blank" rel="noreferrer">{contact.sourceName || "Contact source"}</a><div className="note">Evidence: {contact.evidenceText}</div></div>) : <p className="note">Not found</p>}
